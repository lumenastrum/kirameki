...
- `--workspace <path>` — watch Claude Code and Codex sessions for a specific workspace (useful when Agent Flow is launched from a checkout outside the project you are coding in)
...
If Agent Flow is checked out somewhere other than the project you are coding in, pass the target workspace explicitly:

```bash
pnpm run dev -- --workspace "C:\path\to\your\project"
```

The source dev UI starts on http://localhost:3333 by default so it does not collide with common app dev servers on port 3000. Override it with `--web-port <number>` or `AGENT_FLOW_WEB_PORT`.

On Windows, you can also run `Launch Agent Flow.bat` from the repo root. It starts Agent Flow on http://localhost:3333 and watches the parent workspace by default.

Open http://localhost:3333 and start a Claude Code session in another terminal — events will stream to the browser in real-time.
