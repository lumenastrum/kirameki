...
import { handleAgentSpawn, handleAgentComplete, handleAgentIdle, handlePermissionRequested, handleModelDetected, handleUsageUpdate } from './handle-agent-events'
...
export function processEvent(event: SimulationEvent, prev: SimulationState, ctx: ProcessEventContext): SimulationState {
      ...
      switch (event.type) {
        ...
        case 'usage_update':      handleUsageUpdate(event.payload, state); break
      }
